...
/// A very simple executor: it polls the future in a busy loop until completion (does not allow spawning tasks)
///
...
impl<F: FnMut(usize) -> usize> SchedulingPlan for F {
    fn pick_task(&mut self, num_tasks: usize) -> (usize, bool) {
...
#[derive(Default)]
pub struct CyclingSchedulingPlan {
    index: usize,
}

impl SchedulingPlan for CyclingSchedulingPlan {
    fn pick_task(&mut self, num_tasks: usize) -> (usize, bool) {
        // self.index = (self.index + 1) % num_tasks;
        self.index += 1;
        if self.index >= num_tasks {
...
#[derive(Default)]
pub struct NondetSchedulingPlan;

impl SchedulingPlan for NondetSchedulingPlan {
    #[cfg(kani)]
    fn pick_task(&mut self, num_tasks: usize) -> (usize, bool) {
        let index = kani::any();
...
pub struct NondetFairSchedulingPlan {
    counters: [u8; MAX_TASKS],
    limit: u8,
}

impl NondetFairSchedulingPlan {
    pub fn new(limit: u8) -> NondetFairSchedulingPlan {
        NondetFairSchedulingPlan { counters: [limit; MAX_TASKS], limit }
    }
}

impl SchedulingPlan for NondetFairSchedulingPlan {
    #[cfg(kani)]
    fn pick_task(&mut self, num_tasks: usize) -> (usize, bool) {
...
impl Scheduler {
    ...
    #[inline] // to work around linking issue
    pub fn run(&mut self, mut scheduling_plan: impl SchedulingPlan) {
        let waker = unsafe { Waker::from_raw(NOOP_RAW_WAKER) };
        let cx = &mut Context::from_waker(&waker);
    ...
    pub fn block_on<F: Future<Output = ()> + Sync + 'static>(
        &mut self,
        fut: F,
        scheduling_plan: impl SchedulingPlan,
    ) {
        self.spawn(fut);
